...
out_dir <- "outputs/exploration/shifted_mp_from_gmm_init_em"
...
shifted_mp_density <- function(x, delta, lambda, sigma2) {
  mp_density(x - delta, lambda, sigma2)
}
...
shifted_mp_positive_density <- function(x, delta, lambda, sigma2) {
  shifted_mp_density(x, delta, lambda, sigma2) / mp_positive_mass(lambda)
}
...
moment_start <- function(vals, weights) {
  ...
  q05 <- as.numeric(quantile(vals[weights > median(weights)], 0.05, names = FALSE))
  if (!is.finite(q05)) {
    q05 <- min(vals)
  }
  delta <- max(0, min(q05 * 0.8, mu * 0.5))
  c(delta = delta, lambda = min(max(lambda, 1e-4), 50), sigma2 = max(mu - delta, .Machine$double.eps))
}
...
fit_shifted_mp_weighted <- function(vals, weights, start) {
  ...
  start <- c(delta = as.numeric(start["delta"]), lambda = as.numeric(start["lambda"]), sigma2 = as.numeric(start["sigma2"]))
  ...
  objective <- function(par) {
    ...
    lambda <- exp(par[2])
    sigma2 <- exp(par[3])
    d <- pmax(shifted_mp_positive_density(vals, delta, lambda, sigma2), density_floor)
    -sum(weights * log(d))
  }

  weighted_vals <- vals[weights > median(weights)]
  if (length(weighted_vals) < 10) {
    weighted_vals <- vals
  }
  delta_upper <- max(0, as.numeric(quantile(weighted_vals, 0.20, names = FALSE)) * 0.999)
  starts <- rbind(
    start,
    c(delta = 0, lambda = start["lambda"], sigma2 = weighted_mean(vals, weights)),
    c(delta = delta_upper * 0.25, lambda = start["lambda"], sigma2 = max(weighted_mean(vals, weights) - delta_upper * 0.25, .Machine$double.eps)),
    c(delta = delta_upper * 0.75, lambda = start["lambda"], sigma2 = max(weighted_mean(vals, weights) - delta_upper * 0.75, .Machine$double.eps)),
    c(delta = start["delta"], lambda = start["lambda"] * 0.5, sigma2 = start["sigma2"]),
    c(delta = start["delta"], lambda = start["lambda"] * 2, sigma2 = start["sigma2"]),
    c(delta = start["delta"], lambda = 1, sigma2 = start["sigma2"])
  )
  ...
  fits <- lapply(seq_len(nrow(starts)), function(i) {
    optim(
      ...
      upper = c(delta_upper, log(100), log(max(vals) * 100)),
      control = list(maxit = 1200)
    )
  })

  best <- fits[[which.min(vapply(fits, `[[`, numeric(1), "value"))]]
  out <- c(delta = as.numeric(best$par[1]), lambda = as.numeric(exp(best$par[2])), sigma2 = as.numeric(exp(best$par[3])), objective = best$value)
  if (any(!is.finite(out)) || out["delta"] < 0 || out["lambda"] <= 0 || out["sigma2"] <= 0) {
    out <- c(delta = as.numeric(start["delta"]), lambda = as.numeric(start["lambda"]), sigma2 = as.numeric(start["sigma2"]), objective = NA_real_)
  }
  out
}
...
fit_two_mp_em <- function(vals, init_resp) {
  ...
  for (iter in seq_len(max_iter)) {
    ...
    starts <- lapply(fits, function(fit) c(delta = as.numeric(fit["delta"]), lambda = as.numeric(fit["lambda"]), sigma2 = as.numeric(fit["sigma2"])))

    pi <- colMeans(resp)
    d1 <- pmax(shifted_mp_positive_density(vals, fits[[1]]["delta"], fits[[1]]["lambda"], fits[[1]]["sigma2"]), density_floor)
    d2 <- pmax(shifted_mp_positive_density(vals, fits[[2]]["delta"], fits[[2]]["lambda"], fits[[2]]["sigma2"]), density_floor)
...
make_diagnostics <- function(vals, fit) {
  ...
  curve_df <- rbind(
    data.frame(component = params$component[1], eigenvalue = x_grid, density = params$weight[1] * shifted_mp_positive_density(x_grid, params$delta[1], params$lambda[1], params$sigma2[1])),
    data.frame(component = params$component[2], eigenvalue = x_grid, density = params$weight[2] * shifted_mp_positive_density(x_grid, params$delta[2], params$lambda[2], params$sigma2[2]))
  )
  ...
  log_curve_df <- rbind(
    data.frame(component = params$component[1], log10_eigenvalue = log_grid, density = params$weight[1] * shifted_mp_positive_density(raw_from_log, params$delta[1], params$lambda[1], params$sigma2[1]) * raw_from_log * log(10)),
    data.frame(component = params$component[2], log10_eigenvalue = log_grid, density = params$weight[2] * shifted_mp_positive_density(raw_from_log, params$delta[2], params$lambda[2], params$sigma2[2]) * raw_from_log * log(10))
  )
  log_mix_df <- aggregate(density ~ log10_eigenvalue, log_curve_df, sum)

  write.csv(mix_df, file.path(out_dir, "shifted_mp_from_gmm_model_cdf.csv"), row.names = FALSE)
  write.csv(empirical_df, file.path(out_dir, "shifted_mp_from_gmm_empirical_vs_model_cdf.csv"), row.names = FALSE)
  write.csv(qq_df, file.path(out_dir, "shifted_mp_from_gmm_qq.csv"), row.names = FALSE)
  ...
  raw_density_plot <- ggplot(data.frame(eigenvalue = vals), aes(eigenvalue)) +
    ...
    labs(
      title = "Shifted MP EM initialized from GMM, outliers removed",
      subtitle = "Raw eigenvalue density; black curve is total shifted-MP mixture",
  ...
  log_density_plot <- ggplot(data.frame(log10_eigenvalue = log10(vals)), aes(log10_eigenvalue)) +
    ...
    labs(
      title = "Shifted MP EM initialized from GMM, log10 view",
      subtitle = "Shifted-MP density transformed with the log10 Jacobian",
  ...
  cdf_plot <- ggplot(empirical_df, aes(eigenvalue)) +
    ...
    labs(
      title = "Shifted MP EM CDF, initialized from GMM",
  ...
  qq_plot <- ggplot(qq_df, aes(model, empirical)) +
    ...
    labs(
      title = "Shifted MP EM Q-Q, initialized from GMM",
      x = "Fitted shifted-MP mixture quantile",
      y = "Empirical quantile"
    )

  ggsave(file.path(out_dir, "shifted_mp_from_gmm_density_raw.png"), raw_density_plot, width = 8.5, height = 5.25, dpi = 220)
  ggsave(file.path(out_dir, "shifted_mp_from_gmm_density_log10.png"), log_density_plot, width = 8.5, height = 5.25, dpi = 220)
  ggsave(file.path(out_dir, "shifted_mp_from_gmm_cdf.png"), cdf_plot, width = 8.5, height = 5.25, dpi = 220)
  ggsave(file.path(out_dir, "shifted_mp_from_gmm_qq.png"), qq_plot, width = 6, height = 6, dpi = 220)
}
...
write.csv(fit$parameters, file.path(out_dir, "shifted_mp_from_gmm_parameters.csv"), row.names = FALSE)
write.csv(assign_df, file.path(out_dir, "shifted_mp_from_gmm_assignments.csv"), row.names = FALSE)
write.csv(fit$loglik_trace, file.path(out_dir, "shifted_mp_from_gmm_loglik.csv"), row.names = FALSE)

make_diagnostics(vals, fit)

print(initial_summary)
print(fit$parameters)
message("Wrote shifted MP EM initialized from GMM outputs to ", out_dir)
